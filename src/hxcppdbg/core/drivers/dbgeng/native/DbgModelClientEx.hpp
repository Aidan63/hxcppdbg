...
public:
    ...
    // FunctionValue():
    //
    // Fetches a function value without the overhead of returning field references.
    //
    Object FunctionValue(_In_z_ const wchar_t *functionName) const
    {
        ComPtr<IModelObject> spValue;
        CheckHr(m_spObject->GetRawValue(SymbolFunction, functionName, RawSearchNone, &spValue));
        return Object(std::move(spValue));
    }

    Object FunctionValue(_In_ const std::wstring& functionName) const
    {
        return FunctionValue(functionName.c_str());
    }
